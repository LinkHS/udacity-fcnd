...
class DronewithPID(CoaxialCopter,PIDController_with_ff):
    ...
    def PID_controler_with_estimated_values(self,k_p,k_d,k_i,mass_err,sigma,alpha,use_estimated_height=False):
        ...
        Drone_IMU = self.IMU(Controlled_Drone.X[0], alpha)
        
        # recording the estimated height for each step
        estimated_height_history = Drone_IMU.z_hat
        ...
        for i in range(1,self.z_path.shape[0]-1):
            ...
            if use_estimated_height:
                ...
                z_observation= Drone_IMU.measure(Controlled_Drone.X[0],sigma)
                z_estimated = Drone_IMU.estimate(z_observation)
            ...
            estimated_height_history = np.vstack((estimated_height_history,Drone_IMU.z_hat))
            observation_hostory = np.vstack((observation_hostory,z_observation))
