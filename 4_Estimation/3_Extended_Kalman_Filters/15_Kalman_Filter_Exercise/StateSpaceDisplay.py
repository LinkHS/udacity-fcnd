...
def state_space_display(z,v,mu_0,sigma_0,mu_bar,sigma_bar):
    ...
    predicted_state_space = multivariate_normal([mu_bar[1,0], mu_bar[0,0]], sigma_bar)

    # levels at 1,2 and 3 sigmas 
    predicted_levels= np.array([np.exp(-4.5), np.exp(-2), np.exp(-0.5)])*\
                      predicted_state_space.pdf([mu_bar[1, 0], mu_bar[0, 0]])
    plt.contour(x, y, predicted_state_space.pdf(configuration_space), predicted_levels, colors='Red')

    initial_state_space = multivariate_normal([mu_0[1,0],mu_0[0,0]], sigma_0)
    ...
    plt.scatter(z, v, color='green', label='Initial')
    plt.scatter(mu_bar[1,0], mu_bar[0,0], color='red', label='Predicted')
...
def state_space_display_updated(z,v,mu_0,sigma_0,mu_bar,sigma_bar,mu_updated,sigma_updated):
    ...
    updated_state_space = multivariate_normal([mu_updated[1,0],mu_updated[0,0]], sigma_updated)
    ...
    plt.scatter(z, v, color='Green', label='Initial')
    plt.scatter(mu_bar[1,0], mu_bar[0,0], color='Red', label='Predicted')
    plt.scatter(mu_updated[1,0], mu_updated[0,0], color='black', label='Updated')
